...
#### **Access-Control-Max-Age** **：**表示预检请求 [[Preflight Request](#preflight-request)] 的最大缓存时间。
...
# **1. 返回新的CorsFilter（全局跨域,亲测可行：jdk1.7、springboot1.4）**

在任意配置类（**在项目中随意的位置新建一个如下配置类即可**），返回一个新的CorsFilter Bean，并添加映射路径和具体的CORS配置信息。
...
首先使用[ Spring Initializr ](http://www.jianshu.com/p/d2b08a671e27)快速构建一个Maven工程，什么都不用改，在static目录下，添加一个页面：index.html 来模拟跨域访问。目标地址: <http://localhost:8090/hello>

```html
<!DOCTYPE html><html lang="en"><head>
    <meta charset="UTF-8"/>
    <title>Page Index</title></head><body><h2>前台系统</h2><p id="info"></p></body><script src="webjars/jquery/3.2.1/jquery.js"></script><script>
...
传统文档：[SpringMVC-CORS 使用手册](#mvc-cors)
